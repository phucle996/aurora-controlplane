...
type MfaService struct {
	mfaRepo  iam_domainrepo.MfaRepository
...
func (s *MfaService) Verify(ctx context.Context, challengeID, method, code string) (string, string, error) {
	...
	if challengeID == "" || method == "" || code == "" {
		return "", "", iam_errorx.ErrMfaChallengeInvalid
	}
	...
	if len(values) == 0 {
		return "", "", iam_errorx.ErrMfaChallengeNotFound
	}
	...
	if !slices.Contains(availableMethods, method) {
		return "", "", iam_errorx.ErrMfaMethodNotAllowed
	}
