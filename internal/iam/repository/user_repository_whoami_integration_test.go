...
func TestUserRepositoryGetWhoAmIAggregatesSessionGraph(t *testing.T) {
	...
	mustExecIAM(t, db, `INSERT INTO iam.user_profiles (id, user_id, fullname, avatar_url, bio, timezone, created_at, updated_at)
	...
	mustExecIAM(t, db, `INSERT INTO iam.permissions (id, name, slug, description, created_at)
		VALUES ($1, 'iam:users:read', 'iam:users:read', 'Read users', NOW())`, permReadID)
	mustExecIAM(t, db, `INSERT INTO iam.permissions (id, name, slug, description, created_at)
		VALUES ($1, 'iam:users:write', 'iam:users:write', 'Write users', NOW())`, permWriteID)

	mustExecIAM(t, db, `INSERT INTO iam.user_roles (user_id, role_id) VALUES ($1, $2)`, userID, roleViewerID)
	mustExecIAM(t, db, `INSERT INTO iam.user_roles (user_id, role_id) VALUES ($1, $2)`, userID, roleAdminID)
	mustExecIAM(t, db, `INSERT INTO iam.role_permissions (role_id, permission_id) VALUES ($1, $2)`, roleAdminID, permReadID)
	mustExecIAM(t, db, `INSERT INTO iam.role_permissions (role_id, permission_id) VALUES ($1, $2)`, roleAdminID, permWriteID)
	mustExecIAM(t, db, `INSERT INTO iam.role_permissions (role_id, permission_id) VALUES ($1, $2)`, roleViewerID, permReadID)
